...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            //解析类security注解
            Set<String> baseSecurities = new HashSet<>();
            if (aClass.isAnnotationPresent(Security.class)) {
                ...
                if (values != null && values.length > 0) {
                    baseSecurities.addAll(Arrays.asList(values));
                }
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //解析方法security注解
                if (!baseSecurities.isEmpty()) {
                    handler.getSecurities().addAll(baseSecurities);
                }
                if (method.isAnnotationPresent(Security.class)) {
                    Security methodSecurityAnnotation = method.getAnnotation(Security.class);
                    String[] values = methodSecurityAnnotation.value();
                    if (values != null && values.length > 0) {
...
//        method.invoke() //
        ...
        //权限验证
        if (!preHandle(handler, req.getParameter("username"), resp)) {
            return;
        }
    ...
    //权限验证
    private boolean preHandle(Handler handler, String userName, HttpServletResponse resp) throws IOException {
        if (handler.getSecurities().isEmpty()) {
            return true;
        }
        for (String security : handler.getSecurities()) {
        ...
        resp.getWriter().write(userName + "没有" + handler.getMethod().getName() + "访问权限");
        return false;
    }
